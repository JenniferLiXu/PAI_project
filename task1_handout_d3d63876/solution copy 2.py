...
class Model(object):
    ...
    def __init__(self):
        ...
        kernel = [ ConstantKernel(constant_value=2), Matern, White, 1.0 * RBF(0.05)]
        for i in range(len(kernel))
            self.gpr = GaussianProcessRegressor(kernel = kernel[i], random_state=2021)
            log_marginal_likelihood(theta)
            self.Nyst = Nystroem(gamma=.2, n_components=100)
